 ...
 */
function getTopText () {
    $message = "Du fick ett nytt meddelande skickat via kontaktformuläret på asensor.eu.";
    $message = strlen($message) > 0 ? $message : "You received a new message sent via the contact form on asensor.eu.";
...
function getMailHeaders ($from, $replyTo, $boundary) {
    $headers = array(
        ...
        "Message-ID: <" . implode("@", [md5(uniqid(time())), "asensor.eu"]) . ">",
        "Date: ".date("r (T)"),
        "X_Mailer: PHP/" . phpversion(),
        "X-G1i-WSB-Sendmail: 119.0.10",
    );
...
function getDebugInfo() {
    ...
    if (isValidRequester() || $debugHeader === "asensor.eu") {
